...
> **Last updated:** June 2026 (added env-suffix deviation for mtar/separate-subaccount deployments; corrected estate name TNG → TNS)
...
> **Deviation — mtar deployments into separate subaccounts may omit `<env>`.**
> When DEV/QA/PROD are separate subaccounts (separate spaces, separate domains,
> auto-generated routes), the environment is already encoded by *which subaccount*
> the app is deployed into, so the `<env>` suffix on the CF app name is redundant.
> Apps deployed via `mta.yaml` (mtar transport through the DevOps pipeline) in this
> model use the env-less CF app name `tns-fsm-<capability>-ui`. Local sandbox
> deployments via `manifest.yaml` use a `-sandbox` suffix instead, purely to avoid
> colliding with the pipeline-owned app and route. Example:
> `tns-fsm-inspreppdfviewext-ui` (mtar, any subaccount) and
> `tns-fsm-inspreppdfviewext-ui-sandbox` (local `cf push`). The `<env>`-suffixed
> form remains the default for any single-subaccount, multi-environment deployment.
...
| Inspection Report PDF Viewer | `com.tns.fsm.inspreppdfviewext.app` | `tns-fsm-inspreppdfviewext-ui` (mtar) / `-sandbox` (local) | `tns-fsm-inspreppdfviewext-ui` |
